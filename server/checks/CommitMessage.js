  ...
  async execute(config, hookPayload, token) {
    ...
    if (patterns && Array.isArray(patterns) && patterns.length > 0) {
      ...
      if (badCommits.length === 0) {
        ...
        await this.github.setCommitStatus(owner, name, sha, createStatePayload('All commit messages match at least one configured pattern.'), token)
        ...
        await this.github.setCommitStatus(owner, name, sha, createStatePayload(`${usePlural ? 'Commits' : 'Commit'} ${badSHAs} ${usePlural ? 'do' : 'does'} not match configured patterns.`, 'failure'), token)
      ...
      await this.github.setCommitStatus(owner, name, sha, createStatePayload('No patterns configured to match commit messages against.'), token)
